...
public class PlaneTests {
	...
	public void testIntersectionPoints() {
		...
		Plane plane3 = new Plane(new Point3D(3, 3, -1), new Point3D(5.14, 5.14, -1.71), new Point3D(7, 0, 0),
				new Color(111, 111, 111), new Material(1, 1, 20));
	...
	/*
	 * Map<Geometry, List<Point3D>> intersectionPointsPlane = new
	 * HashMap<Geometry, List<Point3D>>(); Map<Geometry, List<Point3D>>
	 * intersectionPointsPlane2 = new HashMap<Geometry, List<Point3D>>();
	 * Map<Geometry, List<Point3D>> intersectionPointsPlane3 = new
	 * HashMap<Geometry, List<Point3D>>();
	 */

	/*
	 * rayIntersectionPoints.forEach((k, v) -> {
	 * v.addAll(intersectionPointsPlane.get(k) == null ? new
	 * ArrayList<Point3D>() : intersectionPointsPlane.get(k));
	 * intersectionPointsPlane.put(k, v); });
	 */

	// assertTrue(intersectionPointsPlane.get(plane).size() == 9);
	/*
	 * intersectionPointsPlane.forEach((k,v) -> { System.out.println(k + " " +
	 * v); });
	 */
	...
	public void NormalTest() {
		...
		Plane plane = new Plane(point, normal, new Color(111, 111, 111), new Material(1, 1, 20));
